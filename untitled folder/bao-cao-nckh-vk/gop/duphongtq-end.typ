...
#let camera-thuc-te = [camera Raspberry Pi giao tiếp CSI, chụp 640 × 480 rồi đổi kích thước về 224 × 224]
#let dau-ra-am-thanh-thuc-te = [loa mini qua mạch MAX98357A]
#let co-che-am-thanh-thuc-te = [các tệp MP3 tiếng Việt ghi sẵn và phát cục bộ]
...
#let phien-ban-phan-mem = [Python trên Linux ARM64; TensorFlow Lite; `rpicam-jpeg` để chụp ảnh và `mpg123` để phát âm thanh]
#let so-anh-goc = [3.635 ảnh]
#let so-vat-the-doc-lap = [97 vật thể]
#let cach-chia-du-lieu = [80% train, 10% test và 10% validation; một ảnh không xuất hiện đồng thời ở nhiều tập]
#let so-epoch = [17 epoch]
#let kich-thuoc-model = [3,1 MB]
#let so-lop-vat-the = [tiền, và một số vật sắc nhọn]
#let accuracy-train = [91%]
...
#let phan-tich-loi-thuc-te = [rác khác bị nhầm sang vô cơ 60 lần, ; rác hữu cơ bị nhầm sang vô cơ 29 lần; rác vô cơ bị nhầm sang rác khác 20 lần]
...
#let doi-tuong-thu-nghiem = [5 học sinh tham gia mô phỏng trong trạng thái bịt mắt, có sự đồng ý trước khi thử]
#let quy-trinh-thu-nghiem-nguoi-dung = [mỗi người thực hiện 10 lượt với các vật thể đã chuẩn bị; nhóm ghi thời gian và kết quả bỏ rác khi không dùng thiết bị và khi dùng AI-Green Eye]
...
#let han-che-thu-nghiem-nguoi-dung = [người tham gia là học sinh bịt mắt, không phải người khiếm thị; số người và số lượt còn ít nên kết quả chỉ phản ánh thử nghiệm ban đầu]
...
// #text(weight: "bold")[Tính mới của đề tài:] AI-Green Eye kết hợp camera gắn trên vòng đeo đầu, thao tác bằng công tắc, chương trình nhận diện chạy trực tiếp trên thiết bị và phản hồi bằng giọng nói tiếng Việt. Mỗi lượt nhận diện chỉ chụp một ảnh. Hệ thống thử nhận diện 24 vật thể trước; khi chưa đủ tin cậy, chương trình chuyển sang phân loại ba nhóm rác. Toàn bộ quá trình được thực hiện trên thiết bị mà không cần kết nối mạng.

#text(weight: "bold")[Tính mới của đề tài:] AI-Green Eye là thiết bị phân loại rác thải dành riêng cho người khiếm thị và người suy giảm thị lực. Thiết bị được đeo trên đầu nên thuận tiện cho người sử dụng, có thể mở rộng thêm nhiều tính năng hữu ích như tìm đồ vật, hướng dẫn đi đường sau này. Bộ não của thiết bị là model AI được training, dễ dàng nâng cấp chức năng, giao tiếp bằng tiếng Việt.

#text(weight: "bold")[Tính khoa học:] Đề tài sử dụng thị giác máy tính và hai mô hình TensorFlow Lite theo hai tầng. Tầng thứ nhất thuộc họ EfficientNet, nhận diện #(so-lop-vat-the); tầng thứ 2 là MobileNetV2 có kích thước #(kich-thuoc-model), phân biệt rác hữu cơ, vô cơ và rác khác. Bộ dữ liệu ba nhóm gồm 11.056 ảnh sau làm sạch, được chia thành train, validation và test. Nhóm còn thực hiện 1.200 lượt chụp thật trên thiết bị; kết quả tổng hợp đạt 88,08%.

#text(weight: "bold")[Tính thực tiễn:] Sản phẩm có khối lượng #(khoi-luong-tren-kinh), rất nhẹ nên không gây khó chịu khi đeo trên đầu. Sản phẩm hoạt động nhanh, giúp người dùng phân loại rác ngay tại nguồn. Vị trí camera phù hợp để phát triển tính năng tìm đồ vật và hướng dẫn đi đường sau này.
...
Chúng em xin bày tỏ lòng biết ơn sâu sắc đến Ban lãnh đạo cùng  thầy cô giáo Trường THCS Phan Văn Trị đã luôn tạo điều kiện, động viên và giúp đỡ chúng em trong suốt quá trình nghiên cứu, thực hiện đề tài.
...
Chúng em nhận thấy phân loại rác tại nguồn có ý nghĩa thiết thực đối với việc bảo vệ môi trường, nhưng vẫn là một trở ngại đối với người khiếm thị. Do không thể nhìn hoặc nhìn rõ, họ gặp khó khăn khi phân loại rác thải nếu không tiếp xúc trực tiếp với chúng. Qua tìm hiểu và khảo sát tại khu nhà ở dành cho người khiếm thị ở 446 Lý Thái Tổ, chúng em ghi nhận việc chạm trực tiếp để đoán vật liệu vừa bất tiện vừa có nguy cơ gặp cạnh sắc hoặc chất bẩn.
...
Từ câu hỏi trên, chúng em đề xuất thiết bị thông minh mang tên *AI-Green Eye*. Thiết bị được thiết kế dưới dạng vòng đeo trên đầu tích hợp camera góc nhìn trước trán và công tắc điều khiển. Khi thiết bị hoạt động, camera chụp một ảnh cho mỗi lượt, hệ thống nhận diện hai tầng xác định vật thể hoặc nhóm rác rồi phát lời hướng dẫn tương ứng.

= B. CÂU HỎI NGHIÊN CỨU – VẤN ĐỀ NGHIÊN CỨU – MỤC TIÊU VÀ YÊU CẦU

== B1. Câu hỏi nghiên cứu
1. Chúng em có thể xây dựng hệ thống nhận diện hai tầng đủ gọn nhẹ để chạy trên Raspberry Pi Zero 2 W mà không rò rỉ dữ liệu người dùng không?
...
// Chúng em nghiên cứu biện pháp tạo ra một thiết bị đeo ứng dụng trí tuệ nhân tạo nhằm hỗ trợ người khiếm thị phân loại rác sinh hoạt; quá trình xây dựng mô hình nhận diện gọn nhẹ bằng TensorFlow Lite, thiết kế khung vòng đeo trên đầu bằng nhựa in 3D và làm nhãn chữ nổi Braille cho thùng rác để người dùng nhận biết bằng xúc giác.

- Nghiên cứu quá trình xây dựng mô hình AI để xây dựng mô hình nhận diện và phân loại rác thải, nhận diện chính xác một số đồ vật quan trọng như tiền, một số vật sắc nhọn có thể gây nguy hiểm.

- Nghiên cứu các module để lựa chọn các module phù hợp đảm bảo gọn, nhẹ, tiêu thụ ít năng lượng, hiệu quả tốt.

- Tìm hiểu nhu cầu, thói quen của người khiếm thị để thiết kế sản phẩm phù hợp, dễ sử dụng, thoải mái khi đeo.

- Tìm hiểu chữ nổi Braille để hỗ trợ người khiếm thị dễ dàng phân biệt và nhận diện đúng thùng rác.

- Tìm hiểu về in 3D để thiết kế khung vòng đeo trên đầu sao cho nhẹ, ôm vừa vặn và phân bổ đều quanh vòm đầu, dễ thao tác và hạn chế cản trở việc nghe âm thanh xung quanh của người khiếm thị.

== B3. Mục tiêu nghiên cứu
// - Lắp ráp mẫu thử dạng vòng đeo trên đầu có khả năng chụp ảnh và nhận diện rác.
// - Xây dựng hệ thống nhận diện hai tầng: mô hình vật thể nhận diện 24 nhãn cụ thể và mô hình dự phòng phân biệt ba nhóm rác hữu cơ, vô cơ (tái chế) và rác khác; sau đó kiểm tra khả năng hoạt động trên thiết bị thật.
// - Viết chương trình Python kết nối công tắc, camera, hai mô hình nhận diện và loa; đồng thời đo độ trễ của thiết bị.
...
- Tìm hiểu về một số thói quen, nhu cầu của người khiếm thị để thiết kế sản phẩm phù hợp.

- Nghiên cứu về cách training AI để sản phẩm nhận diện và phân loại rác nhanh, trước mắt nhận diện mệnh giá tiền và các vật sắc nhọn.

- Nghiên cứu các module để  có thể nâng cấp chức năng hoặc mở rộng các chức năng mới để phục vụ cho người khiếm thị.
...
3. *An toàn và tôn trọng quyền riêng tư:* Không đưa dữ liệu người dùng công khai lên mạng, sản phẩm khi chạy có nhiệt độ trong giới hạn an toàn và nhiệt độ của vỏ thiết bị phần tiếp xúc với người sử dụng không được lớn hơn nhiệt độ môi trường tối đa $1^0$ C.
// 4. *Chi phí hợp lí:* Chi phí không quá cao để đa số người dùng có thể tiếp cận.

== B5. Giả thuyết, đối tượng và phạm vi nghiên cứu
Từ câu hỏi ban đầu, chúng em đặt giả thuyết rằng: nếu camera được đặt theo hướng nhìn của người dùng, mô hình nhận diện chạy trực tiếp trên thiết bị và kết quả được phát bằng loa ngoài, người dùng có thể phân loại rác nhanh hơn, ít phải chạm trực tiếp vào rác hơn và vẫn giữ được hai tay tự do. Nhãn Braille trên thùng rác là bước đối chiếu bằng xúc giác sau khi nghe hướng dẫn.

Đối tượng nghiên cứu là khả năng nhận diện ba nhóm rác *hữu cơ, vô cơ và rác khác* của mô hình AI khi chạy trên mẫu thử AI-Green Eye, cùng với thời gian phản hồi và khả năng sử dụng của vòng đeo. Phạm vi thử nghiệm gồm bộ dữ liệu của nhóm, các vật thể chuẩn bị cho phép thử độc lập và ba điều kiện ánh sáng đã ghi nhận. Thử nghiệm người dùng mới thực hiện với học sinh bịt mắt; vì vậy, kết quả này chưa thể thay thế cho đánh giá dài hạn của người khiếm thị.
...
1. *Nghiên cứu tài liệu:* Đọc sách về học máy, học sâu, huấn luyện mô hình phân loại ảnh và triển khai mô hình gọn nhẹ trên thiết bị nhúng [1], [2], [3].
2. *Thiết kế và phát triển mẫu thử:* Chia công việc thành từng phần, tìm hiểu cách lập trình và kết nối phần cứng trên Raspberry Pi [4], tra cứu thông số Raspberry Pi Zero 2 W [5] và tài liệu hệ điều hành DietPi [6].
3. *Thực nghiệm và thống kê:* Ghi nhật ký kết quả nhận diện đúng, sai và thời gian xử lý trong các điều kiện ánh sáng khác nhau để xác định những điểm cần cải thiện.
...
Các phương pháp trên được nối với nhau theo một vòng lặp: khảo sát để xác định yêu cầu, thiết kế và chế tạo mẫu thử, đo kết quả, tìm nguyên nhân của lỗi rồi điều chỉnh dữ liệu, chương trình hoặc phần cứng. Nhờ vậy, số liệu không chỉ dùng để báo cáo kết quả mà còn giúp nhóm quyết định phần nào cần sửa.
...
#figure(
  image("tq/image/khao sat.jpg", height: 11cm),
...
Từ kết quả so sánh, chúng em chọn *Phương án 3*: chế tạo thiết bị đeo thông minh sử dụng Raspberry Pi Zero 2 W với thiết kế dạng vòng đeo trên đầu. Giải pháp này giúp người dùng rảnh tay để cầm rác; thao tác chính là bật công tắc và hướng camera trước trán về phía vật thể.
...
- *Giới hạn của máy móc:* Bộ xử lý của chúng em là #(bo-xu-ly-thuc-te), có tài nguyên hạn chế. Vì vậy, nhóm dùng định dạng TensorFlow Lite/LiteRT để giảm kích thước mô hình và triển khai suy luận trên thiết bị [3], [7].
...
#align(center)[
  #table(
    ...
    [2. Phản hồi nhanh],
    [Tính từ lúc bắt đầu một lượt nhận diện đến lúc loa phát ra âm thanh.],
...
== 4.1. Sơ đồ hoạt động
Khi sử dụng AI-Green Eye, thiết bị hoạt động theo quy trình: bật công tắc -> chụp một ảnh -> nhận diện hai tầng -> phát lời hướng dẫn. Bộ xử lý của máy là #(bo-xu-ly-thuc-te), hình ảnh được thu bằng #(camera-thuc-te), còn âm thanh được phát theo cơ chế #(co-che-am-thanh-thuc-te).
...
#figure(
  table(
    ...
    [Bộ xử lý trung tâm], [#(bo-xu-ly-thuc-te)], [#image("tq/image/thiet bi/pi.png")],
    [Camera chụp ảnh], [#(camera-thuc-te)], [#image("tq/image/thiet bi/cam.png", height: 2cm)],
    [Mạch khuếch đại], [MAX98357A], [#image("tq/image/thiet bi/max.png", height: 2.4cm)],
    [Loa ngoài], [Loa mini không che kín hai tai], [#image("tq/image/thiet bi/loa.png", height: 2.2cm)],
    [Bộ phận điều khiển], [Một công tắc vật lý], [#image("tq/image/thiet bi/button.png", height: 2.2cm)],
    [Pin cấp điện], [#(dung-luong-pin)], [#image("tq/image/thiet bi/pin.png", height: 2.5cm)],
    [Khối lượng đo được], [Phần vòng đeo trên đầu: #(khoi-luong-toan-bo)], [#image("tq/image/san pham hoan thien.jpg")],
...
== 4.3. In 3D để lắp ráp linh kiện và lập trình
#figure(
  image("tq/image/in_3d.png", width: 92%),
...
#figure(
  grid(
    columns: (1fr, 1fr),
    gutter: 8pt,
    image("tp/recovered-images/page-006-image-02.png", width: 100%),
    image("tp/recovered-images/page-007-image-01.png", height: 25.4%),

    image("tp/recovered-images/page-007-image-02.png", height: 20%),
    image("tp/recovered-images/page-007-image-03.png", width: 100%),
  ),
...
#figure(
  image("tq/image/san pham 2.jpg", width: 80%),
  caption: [Bố trí phần cứng trên mẫu thử dùng để lập trình.],
  numbering: _ => "4.4",
)
// Tầng nhận diện chi tiết sử dụng 24 nhãn được lưu cùng mã chương trình cũ: các đồng của Việt Nam, bút, chăn, chậu rửa, giẻ lau, đồng hồ, cốc, bát, ấm, chìa khóa, dao nhà bếp, lá cây, bóng đèn, áo, lon kim loại, quả cam/quất, giấy, túi nhựa, vật sắc nhọn nguy hiểm, thìa, thước, bàn chải đánh răng, chai nước và đũa. Danh sách này cho thấy tầng thứ nhất không chỉ phục vụ phân loại rác; trong phạm vi đề tài, kết quả của nó được dùng để gọi tên vật thể và cảnh báo khi phù hợp, còn tầng ba nhóm đảm nhiệm hướng dẫn tới thùng rác.



// Mẫu hoàn thiện khác rõ so với mẫu vòng đeo ban đầu của bài thi cấp thành phố. Ở mẫu cũ, các module còn lộ ra ngoài và dây nối chưa gọn. Sau các lần lắp thử, nhóm thiết kế lại vỏ để cố định camera ở giữa trán và che phần mạch. Số cân mới nhất của mẫu hoàn thiện là 180 g; đây là số liệu được nhóm cân lại để thay cho số 104 g ghi ở bản trước.



== 4.3. Hoạt động của chương trình
Sau khi người dùng bật công tắc, chương trình Python điều khiển các bước còn lại của thiết bị theo quy trình sau:
1. Mạch điện luôn trong trạng thái chờ.
2. Trong mỗi lượt nhận diện, camera chụp một ảnh rồi thay đổi kích thước về 224 × 224 để phù hợp với đầu vào của mô hình.
3. Ảnh được đưa vào mô hình TensorFlow Lite nhận diện 24 vật thể. Chương trình đối chiếu kết quả với ngưỡng tin cậy 80% do chúng em đặt.
4. Nếu tầng vật thể đạt ngưỡng, thiết bị phát tên vật thể và lời hướng dẫn phù hợp. Nếu chưa đạt ngưỡng, cùng ảnh đó được chuyển sang mô hình MobileNetV2 ba lớp để phân loại hữu cơ, vô cơ hoặc rác khác. Việc đổi thứ tự hữu cơ và vô cơ từng xuất hiện trong một bản mã chỉ là thao tác đảo nhãn để kiểm tra, không phải logic của bản sử dụng.
5. Nếu cả hai tầng đều chưa đủ tin cậy, thiết bị nhắc người dùng thực hiện lại lượt nhận diện.
...
Trong quá trình hoàn thiện sản phẩm, chúng em đã nhiều lần điều chỉnh chương trình và thử các cách nhận diện khác nhau. Mô hình MobileNetV2 ba lớp từ giai đoạn trước không bị loại bỏ mà được giữ làm tầng dự phòng vì gọn nhẹ và phù hợp với bộ ba thùng rác thử nghiệm. Sau đó, nhóm bổ sung một mô hình thuộc họ EfficientNet để nhận diện 24 vật thể cụ thể (các đồng của Việt Nam, bút, chăn, chậu rửa, giẻ lau, đồng hồ, cốc, bát, ấm, chìa khóa, dao nhà bếp, bóng đèn,  lon kim loại,  vật có hình dạng sắc nhọn nguy hiểm). Mã chương trình cuối chạy tầng vật thể trước; chỉ khi độ tin cậy dưới 80% mới chuyển sang tầng phân loại ba nhóm. Cách ghép này vừa cho lời báo cụ thể khi mô hình nhận ra vật thể, vừa giữ được khả năng hướng dẫn thùng rác khi vật thể chưa nằm trong nhóm nhận diện chi tiết.

// Hai mô hình đã được chạy trên mẫu thiết bị thật. Tuy nhiên, tài liệu cũ chỉ cho phép xác định mô hình vật thể thuộc họ EfficientNet, chưa đủ căn cứ để khẳng định chính xác biến thể B0. Bộ dữ liệu V11 có trong thư mục lưu trữ cũ không được dùng cho phiên bản này. Vì quá trình huấn luyện thực hiện trên Google Colab và log gốc không còn được lưu, báo cáo chỉ sử dụng các kết quả mà nhóm đã tổng hợp, không bổ sung thêm chỉ số chưa có bằng chứng.
...
#figure(
  image(
    "tq/image/1786641666242_1634650884594342833_6156200798043510897_c426a8e2c12386b73e6d155337abb62c.jpg",
    width: 85%,
  ),
...
#figure(
  image(
    "tq/image/1786641674198_1634650884594342833_6156200798043510897_9bb2f0d8ef9d5dfdbfa4531895927440.jpg",
    width: 88%,
  ),
...
#figure(
  image("tq/image/san pham hoan thien.jpg"),
  caption: [Mẫu thử hoàn chỉnh với vỏ in 3D.],
  numbering: _ => "4.7",
)

== 4.5. Làm nhãn chữ nổi Braille cho thùng rác
Sau khi nghe hướng dẫn, người khiếm thị vẫn cần xác định thùng rác tương ứng. Vì vậy, chúng em làm nhãn chữ nổi Braille và dán lên ba thùng rác thử nghiệm.
...
#align(center)[
  ...
  *Rác khác (other waste):*
  ...
  #align(center)[
    #text(style: "italic")[(Ghi chú: ● là chấm nổi, ◌ là vị trí không có chấm trong ô Braille chuẩn]

    #link("https://vi.wikipedia.org/wiki/Hệ_thống_chữ_nổi_tiếng_Việt")[#text(
      fill: blue,
    )[Nguồn: Hệ thống chữ nổi tiếng Việt – Wikipedia tiếng Việt)]]
  ]]

#figure(
  image("tp/recovered-images/page-009-image-01.png", height: 10cm),
  caption: [Quá trình nhóm làm và kiểm tra nhãn chữ nổi Braille.],
  numbering: _ => "4.8",
)
#figure(
  image("tq/image/thung_rac.png"),
  caption: [Nhãn chữ nổi Braille trên ba thùng rác thử nghiệm.],
  numbering: _ => "4.9",
)

Vì mỗi nơi thùng rác có thể có hình dạng, mầu sắc khác nhau, không có quy định thống nhất về màu sắc của thùng rác nên nhóm quyết định dán nhãn chữ nổi Braille lên ba thùng rác thử nghiệm.
...
+ *Thu thập ảnh:* Nhóm chụp tổng cộng #(so-anh-goc) ảnh của #(so-vat-the-doc-lap) vật thể độc lập và bổ sung thêm ảnh của các vật thể tương ứng từ Internet, (24 vật thể cho mô hình EfficientNet và 3 nhóm cho mô hình MobileNetV2)
...
Mô hình MobileNetV2 ba lớp được huấn luyện trong #(so-epoch) và có kích thước #(kich-thuoc-model). Theo kết quả nhóm đã tổng hợp từ lần chạy trên Google Colab, độ chính xác trên tập huấn luyện tăng từ khoảng 56% lên khoảng 91%, trong khi độ chính xác trên tập xác thực đạt khoảng 79–81% ở các epoch cuối. Loss trên tập huấn luyện giảm còn khoảng 0,26, trong khi validation loss khoảng 0,59.

#figure(
  image("tq/image/training_history.png", width: 16cm),
  caption: [Biểu đồ độ chính xác và sai số từ lần huấn luyện mô hình.],
  //   v(-3mm),
  numbering: _ => "5.1",
)

Biểu đồ cho thấy accuracy của tập train tăng khá đều, nhưng đường validation dao động và không tăng cùng tốc độ. Validation accuracy có lúc đạt 82,6%, sau đó nằm khoảng 79–81% ở các epoch cuối; validation loss cũng không giảm đều như training loss. Chúng em hiểu đây là dấu hiệu mô hình đã học được đặc điểm của ba nhóm rác nhưng khả năng khái quát vẫn còn phụ thuộc vào ảnh và điều kiện chụp. Vì vậy, nhóm không dùng riêng con số 91% của tập train để kết luận mô hình tốt, mà tiếp tục kiểm tra bằng tập test và bằng ảnh mới chụp từ thiết bị.

Ở giai đoạn trước, nhóm đã dùng các biến thể của ảnh gồm xoay, lật ngang, dịch chuyển và phóng to/thu nhỏ để làm ảnh đa dạng hơn; đồng thời dùng early stopping để lưu phiên bản tốt nhất. Cách làm này vẫn được giữ khi thử mô hình mới vì rác thực tế thường bị xoay, móp hoặc chỉ xuất hiện một phần trong khung hình. Tuy nhiên, biến thể của ảnh không thể thay thế ảnh thật trong nhiều điều kiện ánh sáng, nên nhóm vẫn phải bổ sung ảnh chụp thực tế.

#figure(
  grid(
    columns: (1fr, 1fr),
    gutter: 10pt,
    image("tq/image/confusion_matrix.png", width: 100%), image("tq/image/prediction_confidence.png", width: 100%),
  ),
  caption: [Ma trận nhầm lẫn và phân bố độ tin cậy],
  kind: image,
  numbering: _ => "5.2",
)

// Hai biểu đồ trên được giữ như nhật ký của giai đoạn cấp thành phố, không dùng thay cho bảng kiểm tra độc lập 1.200 lượt ở phần sau. Ma trận cũ cho thấy rác vô cơ được nhận diện tốt hơn, còn rác khác dễ bị kéo về lớp vô cơ.
Biểu đồ confidence có phần lớn dự đoán đúng nằm trên ngưỡng 80%, nhưng vẫn có dự đoán sai với độ tin cậy cao. Điều đó cho thấy đặt ngưỡng 80% giúp loại một phần ảnh chưa rõ, nhưng ngưỡng tin cậy không tự sửa được trường hợp mô hình đã học sai đặc điểm; dữ liệu và phép thử độc lập vẫn là phần quyết định. Vì vậy, sau này chúng em đã thử với 1200 mẫu thử độc lập khác với các mẫu dùng để training và validation để đánh giá khả năng nhận diện của thiết bị.

== 5.2. Kết quả nhận diện theo nhóm rác của mô hình đã huấn luyện và chuyển đổi sang tflite
Đây là 1.200 lượt chụp thật và bảng dưới là số liệu nhóm đã tổng hợp sau thử nghiệm. Các phiếu đo ban đầu hiện không còn được lưu do chúng em ghi nhanh vào sổ nhật kí, đồng thời ghi kèm thông tin về ánh sáng môi trường bằng các kí hiệu viết tắt tự quy ước. Vì vậy báo cáo chi nêu  tổng số và kết quả đã chốt. Dưới đây là bảng số liệu và nhận xét của chúng em.
...
Con số #(accuracy-test) là kết quả trên tập test đã chuẩn bị trong bộ dữ liệu; còn 88,08% là kết quả của 1.200 lượt chụp bằng  thiết bị. Khoảng chênh lệch 5,07 điểm phần trăm cho thấy kết quả trên tập dữ liệu chưa phản ánh hết ảnh hưởng của các vật khác ở nền (background của ảnh), ánh sáng, góc cầm chụp và bước chuyển mô hình sang TensorFlow Lite. Vì vậy, khi đánh giá khả năng dùng thực tế, chúng em đánh giá độ chính xác khoảng 88% đến 94% tùy vào điều kiện ánh sáng môi trường, các vật khác trong ảnh.
...
*Phân tích các trường hợp nhận diện sai:* #(phan-tich-loi-thuc-te). Kết quả từ ma trận nhầm lẫn cho thấy nhóm "Khác" có tỉ lệ chính xác thấp nhất ($approx 77,60%$) và là lớp cần được cải thiện nhất (chủ yếu do tập dữ liệu để huấn luyện của chúng em chưa được phong phú do thời gian chụp hình có hạn). Từ những lỗi quan sát được, nhóm chúng em sẽ bổ sung ảnh đa dạng hơn, phù hợp với các góc chụp và điều kiện mà mô hình còn nhận diện chưa tốt để nâng cao hiệu quả phân loại.

// Nhóm “Khác” chứa nhiều vật thể không thuộc hai nhóm còn lại nên hình dạng và bề mặt không đồng nhất. Trong khi đó, một số vật thuộc nhóm này có bề mặt bóng hoặc cạnh thẳng gần giống nhựa, kim loại và bao bì của nhóm vô cơ. Đây là nguyên nhân phù hợp với kiểu nhầm “Khác → Vô cơ” xuất hiện 60 lần. Ngược lại, lớp vô cơ đạt khoảng 93,84%, cao nhất trong ba lớp, cho thấy dữ liệu của lớp này có đặc điểm ổn định hơn trong phép thử. Ma trận chưa chứng minh thiết bị nhận ra mọi loại rác; nó chỉ cho biết hiệu quả với ba nhãn và các vật thể trong phạm vi đã kiểm tra.

== 5.4. Thử nghiệm độ trễ và điều kiện ánh sáng
Chúng em đo thời gian từ lúc bắt đầu một lượt nhận diện đến khi loa phát lời hướng dẫn. Sau #(so-lan-do-do-tre) lần đo, độ trễ trung bình là #(do-tre-trung-binh) giây và lớn nhất là #(do-tre-lon-nhat) giây. Vì sử dụng điện thoại làm đồng hồ bấm giây và thao tác bằng tay nên kết quả đo chưa có độ chính xác cao, tuy nhiên chúng em khá hài lòng với kết quả này.

Bên cạnh độ trễ, chúng em cũng ghi nhận lại điều khiện ánh sáng môi trường tại thời điểm chup ảnh của thiết bị. Kết quả thể hiện trong bảng dưới đây:
...
Kết quả giảm từ 95% ngoài trời sáng xuống 90% trong nhà và 75% khi thiếu sáng. Mức giảm 20 phần trăm giữa hai điều kiện biên cho thấy ánh sáng là yếu tố ảnh hưởng rõ rệt nhất trong phép thử này. Khi thiếu sáng, camera thu được ít chi tiết về màu sắc thay đổi, trong khi mô hình phải dựa vào bề mặt, đường viền và màu của vật thể để xác định vật thế đó.
...
#figure(
  grid(
    columns: (1fr, 1fr),
    gutter: 8pt,
    image("tq/image/thuc_nghiem3.png", height: 22%), image("tq/image/thu nghiem.png", width: 100%),
  ),
  caption: [Hai lượt mô phỏng phân loại rác trong trạng thái bịt mắt.],
  kind: image,
  numbering: _ => "5.3",
)

So với khoảng thời gian gần 45 giây và tỉ lệ bỏ đúng thùng rác sai đến 60% khi chỉ sờ và đoán, thời gian thực hiện chỉ còn 5–7 giây và tỉ lệ bỏ rác đúng lên tới 90% khi dùng thiết bị cho thấy mẫu thử có khả năng hỗ trợ thao tác trong phạm vi bài thử.
// Kết quả thử nghiệm vẫn chưa chứng minh được mức hiệu quả với người khiếm thị thật vì học sinh bịt mắt không có kỹ năng xúc giác và thói quen sinh hoạt giống họ.

#figure(
  image("tq/image/thuc_nghiem2.jpg", width: 92%),
  caption: [Theo dõi kết quả nhận diện và chương trình chạy trực tiếp trên thiết bị.],
  numbering: _ => "5.4",
)

Ảnh theo dõi cho thấy mô hình và chương trình đã được đưa lên thiết bị thay vì chỉ chạy trên máy huấn luyện. Đây là bước kiểm tra cần thiết vì mô hình `.keras` trên máy tính và mô hình `.tflite` trên Raspberry Pi có thể cho kết quả khác nhau sau khi chuyển đổi.

// == 5.6. Chi phí và khả năng triển khai
// Tổng chi phí thực tế của mẫu thử là #(tong-chi-phi-thuc-te), gồm #(danh-sach-linh-kien-va-gia). Camera và module xử lý chiếm phần lớn chi phí. Vì vậy, nếu tiếp tục phát triển, đây là nhóm linh kiện cần ưu tiên tối ưu hoặc tận dụng lại; các chi tiết in 3D và công tắc chỉ chiếm phần nhỏ hơn nhưng quyết định độ vừa vặn và cách thao tác.

// #figure(
//   image("tq/image/gia thanh.png", width: 94%),
//   caption: [Cơ cấu chi phí thực tế của mẫu thử AI-Green Eye.],
//   numbering: _ => "5.6",
// )
...
== 6.1. Tính an toàn
Thiết bị được thiết kế an toàn tuyệt đối cho người sử dụng:
- Không có bộ phận sắc nhọn có thể gây tổn thương.
- Nhiệt độ của vỏ sản phầm, tại ví trí tiếp xúc với người dùng luôn có nhiệt độ bằng với nhiệt độ phòng.
- Hệ thống dùng dòng điện có cường độ rất nhỏ, hiệu điện thế nhỏ nên không xảy ra hiện tượng giật điện.
- Nhiệt độ của CPU lớn nhất đo được là $60^0 C$ khi hoạt động ngoài trời nắng, đây là nhiệt độ rất an toàn trong dải nhiệt độ hoạt động của CPU (thường là $0^0-80^0 C$) nên không xảy ra hiện tượng CPU quá nóng đến mức gây cháy nổ.

== 6.2. Khả năng áp dụng và định hướng phát triển
- *Hiện tại:* Thiết bị thật đã chạy hệ thống hai tầng: nhận diện 24 vật thể cụ thể trước và chuyển sang phân loại ba nhóm hữu cơ, vô cơ, rác khác khi tầng đầu chưa đủ tin cậy. Bộ ba thùng rác thử nghiệm có vị trí quy ước. Thiết bị chưa có tính năng chỉ đường; khả năng nhận diện vật sắc nhọn đã có trong danh sách nhãn nhưng chưa có đủ số liệu riêng để kết luận độ chính xác.

#align(center)[
  #image("tq/image/thuc_nghiem_1.jpg", width: 80%)
  #fig-caption[Hình 6.1. Thử nghiệm nhận diện vật sắc nhọn.]
]

- *Trong tương lai:* Chúng em mong muốn bổ sung dữ liệu và đánh giá riêng cho từng nhãn trong mô hình 24 vật thể, đặc biệt là vật sắc nhọn nguy hiểm và mệnh giá tiền; mở rộng sang một số nhãn thuốc thông dụng và hướng dẫn sử dụng của thuốc đó. Đồng thời nhóm tiếp tục nghiên cứu chức năng chỉ đường cho người khiếm thị.

== 6.3. Hạn chế của đề tài
- Bộ dữ liệu có nhiều ảnh tải từ Internet nên vẫn khác ảnh camera khi sử dụng thật; chênh lệch giữa 94,58% trên tập test và 88,08% ở phép chụp độc lập phản ánh một phần khoảng cách này.
- Lớp “Khác” bao gồm nhiều dạng vật thể, có tỉ lệ đúng thấp nhất và thường bị nhầm sang vô cơ.
- Độ chính xác giảm còn 75% trong điều kiện thiếu sáng.
- Nhóm chưa có điều kiện thử nghiệm tính hiệu quả của sản phẩm với số lượng người tham gia đủ lớn để thống kê và đưa ra đánh giá chính xác về tính hiệu dụng của sản phẩm.
- Nhóm đo độ trễ bằng đồng hồ bấm giờ trên điện thoại nên có sai số thao tác.
// - Quá trình huấn luyện được thực hiện trên Google Colab nhưng log gốc không còn được lưu. Phiếu đo của 1.200 lượt chụp thật cũng không còn; vì vậy báo cáo có thể đối chiếu bảng tổng hợp cuối nhưng chưa thể kiểm tra lại từng lượt.
- Mô hình 24 vật thể đã chạy trên thiết bị thật, nhưng chưa có bảng đánh giá riêng theo từng lớp.
// Mã cũ xác định mô hình thuộc họ EfficientNet nhưng chưa đủ chứng cứ để chốt biến thể cụ thể; bộ dữ liệu V11 không được sử dụng.

// #pagebreak()
// == 6.4. Các biện pháp an toàn
...
// - *Quyền riêng tư:* Camera chỉ chụp một ảnh cho mỗi lượt nhận diện; ảnh được xử lý tại thiết bị và không truyền lên mạng. Bản hoàn thiện đặt yêu cầu xóa ảnh tạm sau mỗi lần nhận diện; nhóm phải kiểm tra bộ nhớ để xác nhận yêu cầu này được thực hiện.

= CHƯƠNG 7. KẾT LUẬN VÀ KIẾN NGHỊ

== 7.1. Kết luận
- Đề tài *"AI-Green Eye"* đã thành công ở bước nhận diện và phân loại rác thải, mô hình có thể được sản xuất và đưa vào sử dụng rộng rãi trong thực tế. AI Green Eye mở ra triển vọng mới trong việc hỗ trợ người khiếm thị trong đời sống hàng ngày.
- Đề tài vẫn còn một số hạn chế do không đủ thời gian và nhân lực để thực hiện như:
  - Nhận diện vật sắc nhọn chưa thật chính xác trong ánh sáng yếu, một số vật có hình thù và màu sắc phức tạp chưa được nhận diện tốt do lượng dữ liệu huấn luyện vẫn còn hạn chế.
  - Số lượng vật thể được đọc tên chính xác còn hạn chế.
...
2. Tiếp tục mở rộng tính năng nhận diện mệnh giá tiền với các loại tiền thông dụng ở Việt Nam như đồng đô la Mỹ và một số ngoại tệ khác, nhận diện và đọc nhãn thuốc thông dụng, hướng dẫn sử dụng của thuốc đó. Tận dụng bộ kit lập trình của MacOS để xây dựng server kết nối MacOS và Pi vì MacOS có nhiều thư viện mạnh mẽ hỗ trợ phát triển Text-To-Speech, Speech-To-Text, Vision, Apple Intelligent,... đủ để hoàn thiện nhiều tính năng nâng cao mà vẫn đảm bảo dữ liệu người dùng không truyền lên mạng internet công cộng.
3. Xin phép thử nghiệm trên quy mô lớn với người khiếm thị để ghi nhận góp ý thiết kế, công dụng, tính năng của sản phẩm.
...
#set par(first-line-indent: 0pt)
#enum(
  [Aurélien Géron, _Hands-On Machine Learning with Scikit-Learn, Keras & TensorFlow_, 2022.],
  [François Chollet, _Deep Learning with Python_, 2021.],
  [Pete Warden và Daniel Situnayake, _TinyML_, 2019.],
  [Simon Monk, tài liệu thực hành Raspberry Pi, 2023.],
  [Raspberry Pi, tài liệu kỹ thuật Raspberry Pi Zero 2 W.],
  [DietPi, tài liệu cài đặt và sử dụng hệ điều hành DietPi.],
  [Google AI Edge, tài liệu LiteRT/TensorFlow Lite.],
  [TensorFlow, hướng dẫn phân loại ảnh.],
  [Mingxing Tan và Quoc V. Le, “EfficientNet: Rethinking Model Scaling for Convolutional Neural Networks”, 2019.],
  [Báo Thanh Niên, bài viết ngày 14/11/2025 về người khiếm thị và người suy giảm thị lực tại Việt Nam.],
)
...
#text(
  style: "italic",
)[Lưu ý: Các đoạn mã dưới đây được giữ để thể hiện quá trình phát triển mô hình ba lớp và các công cụ xử lý dữ liệu. Chúng không phải toàn bộ mã vận hành cuối của hệ thống hai tầng. Logic bản cuối đã được đối chiếu với thư mục mã cũ và được mô tả tại Mục 4.3: công tắc điều khiển, một ảnh cho mỗi lượt, tầng nhận diện 24 vật thể chạy trước và MobileNetV2 ba lớp làm tầng dự phòng.]
...
= Mã huấn luyện MobileNetV2 ở giai đoạn thử trước (train_model.py)
